  ...
  const columns: ColumnDef<AssignedTask>[] = [
    {
      ...
      cell: ({ row }) => (
        <Link
          href={`/workspace/member/tasks/${row.original.id}`}
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <Button asChild size="sm" variant="outline">
          <Link href={`/workspace/member/tasks/${row.original.id}`}>View</Link>
        </Button>
      ),
    },
  ]
      ...
      {tasks.length === 0 ? (
        <Card>
          <CardContent className="pt-6">
            ...
            <Button asChild>
              <Link href="/workspace/member/marketplace">Browse Marketplace</Link>
            </Button>
          </CardContent>
        </Card>
      ) : (
